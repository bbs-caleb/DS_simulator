...
class SemanticSplitter:  # pylint: disable=too-few-public-methods
    ...
    def split(self, text: str) -> List[str]:
        ...
        Chucks are not overlapping group of sentences that are semantically
        coherent.
    ...
    def _split_text(self, text: str) -> List[str]:
        ...
        considering periods, exclamation points, question marks, and newline
        characters as potential separators.
    ...
    def _combine_sentences(self, sentences: List[str]) -> List[str]:
        """
        Combine adjacent sentences to create a context window around each
        sentence.

        This method creates a sliding window of sentences based on the
        specified buffer_back and buffer_forward parameters, allowing each
        sentence to be considered within its surrounding context.
        ...
        -------
        List[str]
            A list of combined sentences, each representing a contextual
            window around an individual sentence.
    ...
    def _calculate_distances(self, embeddings: np.ndarray) -> List[float]:
        """
        Calculate the cosine distances between consecutive sentence
        embeddings.

        This method computes the cosine distance between each pair of
        consecutive embeddings, serving as a measure of semantic
        dissimilarity.
    ...
    def _group_sentences(
        ...
        This method uses the breakpoints determined from the cosine distances
        to split the list of sentences into semantically coherent chunks.
        ...
        breakpoints : List[int]
            Indices indicating where significant semantic shifts occur, used
            as split points.

        Returns
        -------
        List[str]
            A list of semantically coherent text chunks, each composed of
            grouped sentences.
